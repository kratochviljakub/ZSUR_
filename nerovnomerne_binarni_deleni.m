...
for i = 2:R
   ...
   for j = 1:size(tridy_tmp)
       tridy(tridy_tmp(j,3),(((i-2)*2)+1)) = tridy_tmp(j,1);
       tridy(tridy_tmp(j,3),(((i-2)*2)+2)) = tridy_tmp(j,2);
   end
   ...
   if i == R
       ...
       for j = 1:data_size(1) % sestaveni dat pro dalsi deleni
           tmp = isnan(tridy(j,I));
           if tmp == 0
               data_dalsi_deleni(c,1) = tridy(j,I);
               c = c + 1;
           end
       end
...
for i = 1:data_size(1)
    for j = 1:d(2)
        ...
        if (tridy(i,j) ~= 0) & (tmp == 0)
            scatter(data(tridy(i,j),1), data(tridy(i,j),2),[], colors(j,:),'x')
            hold on
        end
    end
end
